...
def write_data(input_file, output_file, features):
    ...
    file_id = input_file.split('/')[-1].split('.')[-3] if '.s' in input_file else input_file.split('/')[-1].split('.')[
        -2]
    file_id = int(file_id)
    cnf, _ = dimacs_to_cnf(input_file.replace('vad_ddnnf_raw', 'vad_raw').replace('nnf', 'cnf'))
...
if __name__ == '__main__':
    ...
    directory_in_str = '../dataset/VAD/vad_ddnnf_raw/'
    directory_in_str_out = '../dataset/VAD/vad_ddnnf/'
