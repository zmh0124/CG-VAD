...
if __name__ == "__main__":
    ...
    if not os.path.exists('../dataset/VAD/vad_raw/'):
        os.mkdir('../dataset/VAD/vad_raw/')
    ...
    for key, anno in tqdm(annotation.items()):
        ...
        cnf_to_dimacs(f'../dataset/VAD/vad_raw/{idx}.cnf', r_container.get_relevant_formula().clauses, num_atom)
        r_sol_t, r_sol_f = find(r_container.get_relevant_formula(), 5, assumptions=[])
        for sol_t_idx in range(len(r_sol_t)):
            cnf_to_dimacs(f'../dataset/VAD/vad_raw/{idx}.st{sol_t_idx}.cnf',
                          [[i] for i in r_sol_t[sol_t_idx]], num_atom)
            cnf_to_dimacs(f'../dataset/VAD/vad_raw/{idx}.sf{sol_t_idx}.cnf',
                          [[i] for i in r_sol_f[sol_t_idx]], num_atom)
        idx += 1
    pk.dump(img_filenames, open(f'../dataset/VAD/vad_raw/idx2filename.pk', 'wb'))
