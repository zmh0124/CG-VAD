...
clauses = pk.load(open(f'../../dataset/VAD/clauses.pk', 'rb'))
objs = pk.load(open(f'../../dataset/VAD/objects.pk', 'rb'))
pres = pk.load(open(f'../../dataset/VAD/predicates.pk', 'rb'))
word_vectors = pk.load(open(f'../../dataset/VAD/word_vectors.pk', 'rb'))
tokenizers = pk.load(open(f'../../dataset/VAD/tokenizers.pk', 'rb'))
variables = pk.load(open(f'../../dataset/VAD/var_pool.pk', 'rb'))
var_pool = formula.IDPool(start_from=1)
for _, obj in variables['id2obj'].items():
    var_pool.id(obj)
converter = Converter(var_pool, pres, objs)
idx2filename = pk.load(open('../../dataset/VAD/vad/idx2filename.pk', 'rb'))
...
train_set = VAD_dataset(train_set_keys, preprocessed_image_features_train, preprocessed_annotation_train, info_train)
train_dataloader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=0)

test_set = VAD_dataset_test(test_set_keys, preprocessed_image_features_test, preprocessed_annotation_test, info_test)
...
def get_formula_from_image(img_name, annotations, embedder, clauses, converter, objs, tokenizers):
    ...
    adj0, features0, labels0, idx_train0, idx_val0, idx_test0, _, _ = \
        load_data(filename=file_id, dataset=args.ds_name, override_path='../../dataset/VAD/', and_or=False)
